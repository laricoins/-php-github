...
class CPSABG {
    ...
    private $gtin;
    public $statusKIZ = ['EMITTED' => 'эмитирован', 'APPLIED' => 'нанесён', 'INTRODUCED' => 'в обороте', 'WRITTEN_OFF' => 'списан', 'RETIRED' => 'выбыл', 'DISAGGREGATION' => 'расформирован', 'WITHDRAWN' => 'выбыл', ];
    ...
    public function WithdrawalFromCirculation($ItemSign, $pg = 'lp') { // pg string + Товарная группа Cм. "Справочник «Список поддерживаемых товарных  lp Предметы одежды, бельё постельное, столовое, туалетное и кухонное
        $document_format = 'MANUAL';
        $type = 'LK_RECEIPT'; // Вывод из оборота JSON
        ...
        */
        ...
        $cmd = '/opt/cprocsp/bin/amd64/cryptcp -sign -thumbprint ' . $this->SHAUSER . ' -strict "' . $file_path . '" "' . $file_path . '.p7s"';
        $output = null;
        $return_var = null;
        ...
        $body = ["document_format" => $document_format, "product_document" => $product_document, "type" => $type, "signature" => $signature, ];
    ...
    public function GetTokenGis() {
        ...
        $cert = $this->SetupCertificate(CURRENT_USER_STORE, "My", STORE_OPEN_READ_ONLY, 0, $this->SHAUSER, 0, 1);
        $signer = new CPSigner();
        ...
        $json = ['uuid' => $authkey->uuid, 'data' => $sm, 'inn' => $this->inn];
        $url = $this->urlV3 . 'true-api/auth/simpleSignIn/'; // Получение аутентификационного токена
        $rez = $client->request('POST', $url, ['debug' => $this->debug_cps, 'curl' => [CURLOPT_SSL_CIPHER_LIST => 'LEGACY-GOST2012-GOST8912-GOST8912', ], 'verify' => WBPRIME_PLUGIN_PATH . 'guc2022.crt', 'json' => ($json), ]);
    ...
    public function CisesSearchGtins($gtins) {
        ...
        $from_date = (new \DateTime('now - ' . CHZ_DAY_MINUS . ' days'))->format(DateTime::ISO8601);
        $headers = ['Authorization' => 'Bearer ' . $token, 'Content-Type' => 'application/json'];
        // Списан выключаем.7
        ...
        while (!$res->isLastPage) {
            $sgtin = end($out);
            $jsonArray = ["filter" => ['gtin' => $gtins, 'productGroups' => ['lp']], "pagination" => ["lastEmissionDate" => $sgtin->emissionDate, "sgtin" => $sgtin->sgtin]];
            $json = json_encode($jsonArray);
            $request = wp_remote_post($url, ['body' => $json, 'headers' => $headers, ]);
            $res = json_decode(wp_remote_retrieve_body($request));
    ...
    public function CisesSearch() {
        ...
        $headers = ['Authorization' => 'Bearer ' . $token, 'Content-Type' => 'application/json'];
        // Списан выключаем.7
        $jsonArray = ["filter" => ['emissionDatePeriod' => ["from" => $from_date, "to" => $to_date], 'productGroups' => ['lp']]];
        $json = json_encode($jsonArray);
        //	print_r($headers); die;
        $request = wp_remote_post($url, ['body' => $json, 'headers' => $headers, ]);
        ...
        while (!$res->isLastPage) {
            $sgtin = end($out);
            $jsonArray = ["filter" => ['emissionDatePeriod' => ["from" => $from_date, "to" => $to_date], 'productGroups' => ['lp']], "pagination" => ["lastEmissionDate" => $sgtin->emissionDate, "sgtin" => $sgtin->sgtin]];
            $perPage++;
            $json = json_encode($jsonArray);
            $request = wp_remote_post($url, ['body' => $json, 'headers' => $headers, ]);
            $res = json_decode(wp_remote_retrieve_body($request));
    ...
    public function CodesCheck($codes, $inn) {
        ...
        $gtin = '04640255271761';
        $url = 'https://suzgrid.crpt.ru/api/v3/order/status?orderId=' . $orderId . '&omsId=' . $this->omsId . '&gtin=' . $gtin;
        $url = 'https://suzgrid.crpt.ru//api/v3/codes?orderId=' . $orderId . '&omsId=' . $this->omsId . '&gtin=' . $gtin;
        $url = 'https://suzgrid.crpt.ru/api/v3/order/codes/blocks?omsId=' . $this->omsId . '&orderId=' . $orderId . '&gtin=' . $gtin;
        /*
        ...
        */
        ...
        $json = ["filter" => ["orderIds" => ['13b8b729-e353-43c6-a58e-13d3816290c3'], 'workflowTypes' => ['GET_CODES']], 'limit' => 100];
        $url = 'https://suzgrid.crpt.ru/api/v3/receipts/receipt/search?omsId=' . $this->omsId;
        $res = $client->request('POST', $url, ['debug' => 0, 'curl' => [CURLOPT_SSL_CIPHER_LIST => 'LEGACY-GOST2012-GOST8912-GOST8912', ], 'json' => $json, 'verify' => WBPRIME_PLUGIN_PATH . 'guc2022.crt', ]);
        echo '888';
    ...
    public function LpOrdersNotSign($products, $productionOrderId) {
        $jsonArray = ["productGroup" => "lp", "products" => $products, 'releaseMethodType' => 'PRODUCTION', 'createMethodType' => $this->createMethodType, "productionOrderId" => $productionOrderId];
        $url = 'https://suzgrid.crpt.ru/api/v2/lp/orders?omsId=' . $this->omsId;
        $json = json_encode($jsonArray);
        $headers = ['Accept' => 'application/json', 'clientToken' => $this->token, 'Content-Type' => 'application/json'];
        $request = wp_remote_post($url, array('body' => $json, 'headers' => $headers,));
        $logger = wc_get_logger();
        if (is_wp_error($request)) {
            // TODO: validate the error.
            $logger->info(wc_print_r($request, true), array('source' => 'CPSABG'));
    ...
    public function LpOrdersSign($products, $productionOrderId) {
        $jsonArray = (object)["productGroup" => "lp", "products" => $products, 'attributes' =>['releaseMethodType' => 'PRODUCTION', 'createMethodType' => $this->createMethodType, "productionOrderId" => $productionOrderId]
...
"productionOrderId": "08528091-808a-41ba-a55d-d6230c64b332"
		...
		*/
        ...
        $cmd = '/opt/cprocsp/bin/amd64/cryptcp -sign -thumbprint ' . $this->SHAUSER . ' -detached -strict "' . $file_path . '" "' . $file_path . '.p7s"';
        $output = null;
        $return_var = null;
        exec($cmd, $output, $return_var);
   //     print_r($output);
   //     print_r($return_var);
        ...
        //	die;
        $headers = ['Content-Type' => 'application/json', 'Accept' => 'application/json', 'clientToken' => $this->token, 'X-Signature' => $signature];
  /*      print_r($url);
        print_r("\n");
        print_r($json);
        print_r("\n");
	...
	*/
    ...
    public function LpOrders($products) { // делаем заказ ордера 01.01.00.00 Создать заказ на эмиссию КМ».
        $jsonArray = ["productGroup" => "lp", "products" => $products, 'releaseMethodType' => 'PRODUCTION', 'createMethodType' => 'SELF_MADE'];
        $url = 'https://suzgrid.crpt.ru/api/v2/lp/orders?omsId=' . $this->omsId;
        //  $headers = ['clientToken' => $this->token,  'Content-Type' => 'application/json'];
        $json = json_encode($jsonArray);
        ...
        print_r("\n");
        $cert = $this->SetupCertificate(CURRENT_USER_STORE, "My", STORE_OPEN_READ_ONLY, 0, $this->SHA1, 0, 1);
        $signer = new CPSigner();
        $signer->set_Certificate($cert);
        $sd = new CPSignedData();
    ...
    public function GetCodesAbg($orderId, $gtin, $quantity) {
        $headers = ['clientToken' => $this->token, 'Content-Type' => 'application/json'];
        $url = 'https://suzgrid.crpt.ru/api/v3/codes?orderId=' . $orderId . '&gtin=' . $gtin . '&quantity=' . $quantity . '&omsId=' . $this->omsId;
    ...
    public function SearchMarketSku($sku) { // тут опускаем маркет SKU   оно ищет по ЧЗ поле article и возвращает 1 ый найденный
    ...
    public function OrderList() { // доработать
        // $headers = [ 'Authorization' => 'clientToken ' .$this->token];
        $headers = ['clientToken' => $this->token, 'Content-Type' => 'application/json'];
        $client = new GuzzleHttp\Client(['headers' => $headers]);
        $url = 'https://suzgrid.crpt.ru/api/v3/order/list?omsId=' . $this->omsId;
        $res = $client->request('GET', $url, ['debug' => $this->debug_cps, 'curl' => [CURLOPT_SSL_CIPHER_LIST => 'LEGACY-GOST2012-GOST8912-GOST8912', ], 'verify' => WBPRIME_PLUGIN_PATH . 'guc2022.crt', ]);
    ...
    public function GetProductListGtin() {
        ...
        //ceil(405/10)
        $client = new GuzzleHttp\Client();
        $searchStr = $this->urlNK . '/v4/product-list?apikey=' . $this->apikey . '&limit=' . $this->limit_GetProductListGtin . '&offset=' . $offset . '&from_date=' . $from_date . '&to_date=' . $to_date . '&good_status=published';
        $res = $client->request('GET', $searchStr, ['debug' => $this->debug_cps])->getBody()->getContents();
        $res = json_decode($res);
        ...
        for ($offset = 1;$offset <= ceil($total / $this->limit_GetProductListGtin);$offset++) {
            $searchStr = $this->urlNK . '/v4/product-list?apikey=' . $this->apikey . '&limit=' . $this->limit_GetProductListGtin . '&offset=' . $offset * $this->limit_GetProductListGtin . '&from_date=' . $from_date . '&to_date=' . $to_date . '&good_status=published';
            //	print_r($searchStr);
            $res = $client->request('GET', $searchStr, ['debug' => $this->debug_cps])->getBody()->getContents();
            $res = json_decode($res);
            if ($res->result->goods) {
    ...
    public function GetProductListGtinAddArticle($goods) {
        ...
        if (!$res) {
            ...
            $searchStr = $this->urlNK . '/v3/product?apikey=' . $this->apikey . '&gtins=' . $gtins;
            $client = new GuzzleHttp\Client();
            $res = $client->request('GET', $searchStr, ['debug' => $this->debug_cps])->getBody()->getContents();
            $res = json_decode($res);
            wp_cache_set($cache_key, $res, 'GetProductListGtinAddArticle', WBPRIME_CACHE_CHZ_TIME);
        }
